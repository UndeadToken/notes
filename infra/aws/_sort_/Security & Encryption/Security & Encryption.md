...
aws kms encrypt --key-id alias/tutorial --plaintext [fileb://example.txt](fileb://example.txt) --output text --query CiphertextBlob --region eu-west-2 > example-encrypted.base64  
  
cat example-encrypted |base64 > example-encrypted  
  
certutil -code ./example-encrypted.base64 . example-encrypted  
  
aws kms decrypt --cyphertext-blob [files://example-encrypted](files://example-encrypted) --output text --query Plaintext > example-decrypted.base64 --region eu-west-2  
